...
def main():
    ...
    if len(sys.argv) == 1 and __file__ == '<stdin>':
        ...
        auto_disconnect.disconnect()
        config_path = os.path.join(os.path.dirname(__file__), 'odoo.conf')
        if os.path.isfile(config_path) and '-c' not in sys.argv and '--config' not in sys.argv:
