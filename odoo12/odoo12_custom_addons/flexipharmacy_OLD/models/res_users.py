...
class ResUsers(models.Model):
    ...
    # pos_user_type = fields.Selection([('delivery_user', 'Delivery User')],string="POS User Type", default='salesman')
    ...
    @api.onchange('login_with_pos_screen', 'default_pos')
    def onchange_lifetime_card(self):
        if not self.login_with_pos_screen:
            self.default_pos = False
        if self.login_with_pos_screen and self.default_pos:
            user = self.search([('login', '=', self.login)])
            current_session = self.env['pos.session'].search([
                ('user_id', '=', user.id),
                ('state', 'not in', ['closed']),
                ('rescue', '=', False)
            ])
            if current_session and current_session.config_id \
                    and current_session.config_id.id != self.default_pos.id:
                raise UserError(_("Please close the existing POS session, before assigning a new one!"))
