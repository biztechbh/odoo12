...
class printer_Server(models.Model):
    _name = 'printer.server'
    _description = 'Printer server'

    name = fields.Char(default='Localhost', required=True, help='Name of the server.')
    address = fields.Char(default='localhost', required=True,
    ...
    printer_ids = fields.One2many(comodel_name='printer.printer', inverse_name='server_id',
                                  string='Printers List', help='List of printers available on this server.')
    ...
    def update_printers(self, domain=None, raise_on_error=False):
        ...
        for server in servers:
            ...
            server.printer_ids.filtered(lambda record: record.system_name not in updated_printers)\
    ...
    def update_jobs(self, which='all', first_job_id=-1):
        job_obj = self.env['printer.job']
        printer_obj = self.env['printer.printer']
