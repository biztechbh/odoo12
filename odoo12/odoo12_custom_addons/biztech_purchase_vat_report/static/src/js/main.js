odoo.define('biztech_purchase_vat_report.main', function (require) {
    "use strict";
    ...
    var PurchaseVatReport = AbstractAction.extend(ControlPanelMixin, {
        title: core._t('Purchase VAT Report'),
        template: 'PurchaseVatReport',
        events: {
        ...
        export_as_excel : function(){
            ...
            rpc.query({
                model: 'pos.order',
                method: 'export_as_excel_report_purchase_vat',
                args: [[], context.from_date ? context.from_date : false, context.to_date ? context.to_date : false, context.categ_id ? context.categ_id : false, context.product_id ? context.product_id : false],
					...
					message: '<img src="/biztech_purchase_vat_report/static/src/img/loading_icon.gif" width="150" height="150" style="border:unset !important;"/>'
                ...
                if (res && res.report_id){
                    ...
                    self.do_action({
                        ...
                        res_model: 'report.purchase.vat.wizard.download',
                        views: [[false, 'form']],
                        target: 'new',
        ...
        export_as_pdf : function(){
            ...
            rpc.query({
                model: 'pos.order',
                method: 'export_as_pdf_report_purchase_vat',
                args: [[], context.from_date ? context.from_date : false, context.to_date ? context.to_date : false, context.categ_id ? context.categ_id : false, context.product_id ? context.product_id : false],
                },$.blockUI({
					css: {
						cursor:'wait',
                ...
                if (res){
                    $.unblockUI();
                    self.do_action('biztech_purchase_vat_report.action_purchase_vat_dashboard_report',{additional_context:{
                        active_ids:[res],
                    }})
                }
            });
        ...
        reopen_report : function(){
            var self = this;
            self.do_action('biztech_purchase_vat_report.action_purchase_vat_dashboard_wizard')
        },
        renderElement: function () {
            ...
            if (context.from_date && context.to_date){
                ...
                $(self.$el.find('.report_name')).html('<span><b>Purchase VAT Report</b></span>');
                if (context.from_date && context.to_date ){
                ...
                rpc.query({
                    ...
                    args: [[], context.from_date ? context.from_date : false, context.to_date ? context.to_date : false, context.categ_id ? context.categ_id : false, context.product_id ? context.product_id : false],
                        ...
                        if (res){
                        	...
                        	var g_tot_qty = 0;var g_tot_rate = 0;var g_tot_vat = 0;var g_tot_amt = 0;
								var columns = [
									  {"name":"date_order","title":"Date"},
									  {"name":"number","title":"Invoice No."},
									  {"name":"vendor_name","title":"Vendor Name"},
									  {"name":"vendor_tin","title":"Vendor TIN"},
								...
								html += '</h4>';
								html += '</div>';
								html += '<div class="panel-collapse collapse show">';
								html += '<div class="card-body">';
								html += '<table class="table table-bordered table-hover purchase_vat_report per_table" data-paging="true" data-filtering="true" data-sorting="true"/>'
								html += '</div>';
								html += '</div>';
								html += '</div>';
							...
							var selected_list = [];
							var g_selected_list = [];
                            self.$el.find('.purchase_vat_report').footable({
                                "columns": columns,
                                "rows": res,
                                'on': {
    ...
    core.action_registry.add('open_purchase_vat_report_view', PurchaseVatReport);
    return {
