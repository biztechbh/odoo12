odoo.define('biztech_sale_profit_report.main', function (require) {
    "use strict";
    ...
    var SalesProfitReport = AbstractAction.extend(ControlPanelMixin, {
        title: core._t('Sales Profit Report'),
        template: 'SalesProfitReport',
        events: {
        ...
        export_as_excel : function(){
            ...
            rpc.query({
                model: 'pos.order',
                method: 'export_as_excel_report_sales_profit',
                args: [[], context.from_date ? context.from_date : false, context.to_date ? context.to_date : false, context.categ_id ? context.categ_id : false, context.product_id ? context.product_id : false, context.shop_id ? context.shop_id : false],
					...
					message: '<img src="/biztech_sale_profit_report/static/src/img/loading_icon.gif" width="150" height="150" style="border:unset !important;"/>'
                ...
                if (res && res.report_id){
                    ...
                    self.do_action({
                        ...
                        res_model: 'report.sale.profit.wizard.download',
                        views: [[false, 'form']],
                        target: 'new',
        ...
        export_as_pdf : function(){
            ...
            rpc.query({
                model: 'pos.order',
                method: 'export_as_pdf_report_sales_profit',
                args: [[], context.from_date ? context.from_date : false, context.to_date ? context.to_date : false, context.categ_id ? context.categ_id : false, context.product_id ? context.product_id : false, context.shop_id ? context.shop_id : false],
                },$.blockUI({
					css: {
						cursor:'wait',
                ...
                if (res){
                    $.unblockUI();
                    self.do_action('biztech_sale_profit_report.action_sales_profit_dashboard_report',{additional_context:{
                        active_ids:[res],
                    }})
                }else{
                    $.unblockUI();
        ...
        reopen_report : function(){
            var self = this;
            self.do_action('biztech_sale_profit_report.action_sales_profit_dashboard_wizard')
        },
        renderElement: function () {
            ...
            if (context.from_date && context.to_date){
                ...
                $(self.$el.find('.report_name')).html('<span><b>Sales Profit Report</b></span>');
                if (context.from_date && context.to_date ){
                ...
                rpc.query({
                    model: 'pos.order',
                    method: 'get_sales_profit_data',
                    args: [[], context.from_date ? context.from_date : false, context.to_date ? context.to_date : false, context.categ_id ? context.categ_id : false, context.product_id ? context.product_id : false, context.shop_id ? context.shop_id : false],
                    },{async: false}).then(function (res) {
                        if (res){
                        	$.unblockUI();
                        	var g_tot_inv_amt = 0;var g_tot_cost = 0;var g_tot_profit = 0;var g_tot_profit_per = 0;
								var columns = [
									  {"name":"date_order","title":"Date"},
									  {"name":"invoice_amount","title":"Invoice Amount","type": "number","decimalSeparator": ".","thousandSeparator":","},
								...
								var class_name = 'sales_profit_report';
    ...
    core.action_registry.add('open_sales_profit_report_view', SalesProfitReport);
    return {
        SalesProfitReport: SalesProfitReport,
    };
});
