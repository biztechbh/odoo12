...
class PosConfig(models.Model):
    ...
    pos_assigned_user = fields.Char(compute='_get_assigned_user')

    min_percentage_field = fields.Float('Min %age field')

    @api.multi
    def _get_assigned_user(self):
        """
        Gets the user to whom the given config is been assigned
        """
        for pos_config in self:
            assigned_user = self.env['res.users'].search([
                ('default_pos', '=', pos_config.id)
            ])
            pos_config.pos_assigned_user = assigned_user.name or ""
    ...
    def open_session_cb(self):
        ...
        """
        self.ensure_one()
        if self._context.get('from_aspl_direct_login') \
                and self._context['from_aspl_direct_login'] == True:

            if self.env.user.login_with_pos_screen \
                    and self.env.user.default_pos.id != self.id:
                raise UserError(_("You can not open the session, which is not assigned to you!"))

            if not self.env.user.login_with_pos_screen \
                    and self.env['res.users'].search([
                ('default_pos', '=', self.id)
            ]):
    ...
    def name_search(self, name, args=None, operator='ilike', limit=100):
        """
        To show the only those available config,
            which are not being used by any other user,
            which are not assigned to any other user
            or the one which is used by the given user him/herself.
        """

        if self._context.get('from_users'):
            unused_config = []
            user_arg = []

            if self._context.get('res_user'):
                user_arg = [('id', '!=', self._context.get('res_user'))]
            other_users = self.env['res.users'].search(user_arg)

            assigned_configs = other_users.mapped('default_pos')

            all_config = self.env['pos.config'].search([])
            available_config = all_config - assigned_configs

            all_session = self.env['pos.session'].search([])
            if len(all_session) > 0:
                open_session = all_session.filtered(
                    lambda r: r.state not in ['closed'] and not r.rescue
                )

                if self._context.get('res_user'):
                    req_user = self.env['res.users'].browse(
                        self._context.get('res_user')
                    )
                    open_session = open_session.filtered(
                        lambda r: r.user_id not in [req_user]
                    )

                open_config = open_session.mapped('config_id')
                available_config = available_config - open_config

            unused_config = [conf.id for conf in available_config]

            args += [('id', 'in', unused_config)]
        return super(PosConfig, self).name_search(name, args=args,
    ...
    def on_change_multi_shop_id(self):
        if self.multi_shop_id:
            ...
            self.picking_type_id.write({'default_location_src_id': self.multi_shop_id.location_id.id})

    @api.multi
    def write(self, vals):
        if vals.get('module_pos_restaurant'):
            raise Warning(_("You Can't Use Restaurant While Using FlexiPharmacy!"))
        if not((vals.get('enable_order_discount') and not vals.get('module_pos_discount')) or \
                (vals.get('module_pos_discount') and not vals.get('enable_order_discount')) or \
                (not vals.get('module_pos_discount') and not vals.get('enable_order_discount'))):
            raise Warning(_("You Can't Use Both the Discount Feature in same time While Using FlexiPharmacy!"))
        if self.enable_order_discount and vals.get('module_pos_discount') and vals.get('enable_order_discount'):
            raise Warning(_("You Can't Use Both the Discount Feature in same time While Using FlexiPharmacy!"))
        if self.module_pos_discount and vals.get('enable_order_discount') and vals.get('module_pos_discount'):
            raise Warning(_("You Can't Use Both the Discount Feature in same time While Using FlexiPharmacy!"))
    ...
    def create(self, values):
        if values.get('module_pos_restaurant'):
            raise Warning(_("You Can't Use Restaurant While Using FlexiPharmacy!"))
        if values.get('enable_order_discount') and values.get('module_pos_discount'):
            raise Warning(_("You Can't Use Both the Discount Feature in same time While Using FlexiPharmacy!"))
    ...
    enable_product_set_uom = fields.Boolean(string="Set Product UOM")
    # Custom Global Discount Feature
    enable_order_discount = fields.Boolean(string = "Order Discount")
    order_discount_product = fields.Many2one('product.product', string = "Order Discount Product")
...
class pos_order(models.Model):
    ...
    def _create_account_move_line(self, session=None, move=None):
        ...
        for each in all_lines:
            line = each[2]
            credit_total += line.get('credit')
            debit_total += line.get('debit')

        difference_amount = debit_total - credit_total
        amount_difference_account_id = int(
            self.env['ir.config_parameter'].sudo().get_param('flexipharmacy.pos_amount_difference_account_id', 24))

        if difference_amount > 0:
            all_lines.append((0, 0,
                              {'name': 'Difference Amount',
                               'account_id': amount_difference_account_id,
...
class pos_session(models.Model):
    ...
    @api.multi
    def action_pos_session_validate_picking(self):
    ...
    def _validate_pickings(self, order):
        ...
        if pickings:
            if order.picking_id.move_lines.filtered(lambda l: l.reserved_availability == 0.0):
                for line in order.picking_id.move_line_ids.filtered(lambda ml: ml.picking_id):
                    product = line.product_id
                    if product and product.tracking != 'none':
                        if not line.lot_name or not line.lot_id:
                            line.lot_name = '123-' + str(line.id)
                        if not line.qty_done:
                            qty = line.product_uom_qty
                            line.qty_done = qty
                immediate_transfer = self.env['stock.immediate.transfer'].create(
                    {'pick_ids': [(4, order.picking_id.id)]})
                immediate_transfer.with_context({'validate_pickings': True}).process()
                for move in order.picking_id.move_lines:
                    if len(move.move_line_ids.ids) == 1:
                        if move.move_line_ids.qty_done > 0 and not move.move_line_ids.picking_id:
                            move.move_line_ids.write({'picking_id': order.picking_id.id})
            else:
                if order.picking_id and order.picking_id.move_line_ids:
                    for line in order.picking_id.move_line_ids.filtered(lambda ml: ml.picking_id):
                        product = line.product_id
                        if product and product.tracking != 'none':
                            if not line.lot_name or not line.lot_id:
            ...
            waiting_move_lines = order.picking_id.move_lines.filtered(
                lambda l: l.reserved_availability < l.product_uom_qty and l.quantity_done < l.product_uom_qty)
            if waiting_move_lines:
                for line in waiting_move_lines:
                    product = line.product_id
                    if line.lot_number and len(line.lot_number) > 0:
                        lot_id = self.env['stock.production.lot'].search(
                            [('name', 'ilike', line.lot_number), ('product_id', '=', product.id)])
                    else:
                        lot_id = self.env['stock.quant'].search(
                            [('product_id', '=', product.id), ('location_id.usage', '=', 'internal')], limit=1,
                            order="id ASC").lot_id
                        if not lot_id:
                            lot_id = self.env['stock.production.lot'].create({'name': '1234-' + line.name,
                    ...
                    required_qty = line.product_uom_qty - line.reserved_availability \
                        if line.reserved_availability != 0 else line.product_uom_qty - line.quantity_done
                    inventory_id = self.env['stock.inventory'].create({'company_id': self.env.user.company_id.id,
                                                                       'date': datetime.now(),
                                                                       'filter': 'lot',
                                                                       'name': line.name,
                                                                       'lot_id': lot_id.id if lot_id else False,
                                                                       'line_ids': [(0, 0, {'product_id': product.id,
                                                                                            'location_id': self.env.ref(
                                                                                                'stock.stock_location_stock').id,
                                                                                            'prod_lot_id': lot_id.id if lot_id else False,
                                                                                            'product_qty': 0.0})]})
                    inventory_id.write({'state': 'confirm', 'date': fields.Datetime.now()})
                    for each_line in inventory_id.line_ids:
                        each_line.write({'product_qty': each_line.theoretical_qty + required_qty
                        if each_line.theoretical_qty >= 0 else required_qty})
            ...
            remaining_done_moves = order.picking_id.move_lines.filtered(lambda l: l.quantity_done < l.product_uom_qty)
            if remaining_done_moves:
                for move in remaining_done_moves:
                    done_move_ids = [each.id for each in move.move_line_ids if each.qty_done > 0]
                    if len(move.move_line_ids.ids) > 1 and done_move_ids:
                        self.env['stock.move.line'].browse((done_move_ids[1:])).unlink()
    ...
    @api.model
    def get_session_report(self, shop_id):
        shop_domain = ""
        try:
            #             sql query for get "In Progress" session
            if shop_id:
                shop_domain = "and pc.multi_shop_id=%s" %(shop_id)
            session_detail_query = """
                select ps.id,pc.name, ps.name from pos_session ps
                left join pos_config pc on (ps.config_id = pc.id)
                where ps.state='opened' %s
            """%(shop_domain)
            self._cr.execute(session_detail_query)
            ...
            """% (shop_domain)
            self._cr.execute(session_total_query)
            ...
            for pay_id in session_detail:
                self._cr.execute("""
                    ...
                    where pos_session_id=%s %s
                """ % (pay_id[0], shop_domain))
            ...
            for cate_id in session_detail:
                self._cr.execute("""
                    select ppc.name, sum(pol.price_unit), pc.name from pos_category ppc
                    join product_template pt on ppc.id = pt.pos_categ_id
                    join product_product pp on pt.id = pp.product_tmpl_id
                    join pos_order_line pol on pp.id = pol.product_id
                    join pos_order po on pol.order_id = po.id
                    join pos_session ps on ps.id = po.session_id
                    join pos_config pc ON ps.config_id = pc.id
                    where po.session_id = %s %s
                    group by ppc.name, pc.name
                """ % (cate_id[0], shop_domain))
            ...
            for cate_id_null in session_detail:
                self._cr.execute(""" 
                    select sum(pol.price_unit), pc.name from pos_order_line pol
                    join pos_order po on po.id = pol.order_id
                    join product_product pp on pp.id = pol.product_id
                    join product_template pt on pt.id = pp.product_tmpl_id
                    join pos_session ps on ps.id = po.session_id
                    join pos_config pc on ps.config_id = pc.id
                    where po.session_id = %s and pt.pos_categ_id is null %s
                    group by pc.name
                """ % (cate_id_null[0], shop_domain))
