        ...
        barcode_product_action: function(code){
            ...
            setTimeout(function(){
                if(code.code.length == 8){
                    var product_code = code.code.slice(0,4)
                    var product_lot_code = code.code.slice(4,8)
                }else if(code.code.length == 7){
                    var product_code = code.code.slice(0,4)
                    var product_lot_code = code.code.slice(4,7)
                }
                code.product_code = product_code
                code.product_lot_code = product_lot_code
                self.pos.scan_product(code).then(function(result){
                    if (result) {
        ...
        renderElement: function() {
            ...
            this.$('.pay').unbind('click').click(function(){
                ...
                for(var i=0 ; i < order.orderlines.length ; i++){
                    ...
                    var orderline = order.orderlines.models[i];
                    if (!((((orderline.product.standard_price+(orderline.product.standard_price*self.pos.config.min_percentage_field/100)))*orderline.get_quantity()) < orderline.get_all_prices().priceWithoutTax)) {
                        self.gui.show_popup('error',{
                            'title': _t('Error: Margin Error'),
                            'body': _("Selling price less than the allowed margine"),
                        });
                        return
                    }
                }
            ...
            if(!order.get_ret_o_id() && order.get_sale_mode() && self.pos.config.enable_card_charges && self.pos.get_cashier().access_card_charges){
		        if (order.selected_paymentline && order.selected_paymentline.cashregister.journal.apply_charges) {
		        	if(order.selected_paymentline.cashregister.journal.fees_type === _t('percentage')){
		        		charge = self.format_currency_no_symbol((order.selected_paymentline.get_amount() * order.selected_paymentline.cashregister.journal.fees_amount) / 100);
		        	} else if(order.selected_paymentline.cashregister.journal.fees_type === _t('fixed')){
		        		charge = self.format_currency_no_symbol(order.selected_paymentline.cashregister.journal.fees_amount);
		        	}
		        	order.selected_paymentline.set_payment_charge(charge);
		        }
	        }
...
//				line.set_line_loyalty_amount(self.get_loyalty_amount_by_point(line_points));
    	...
    	render_orderline: function(orderline){
	        ...
	        var product_uom = el_node.querySelector('.product_uom');
            if(product_uom){
            	product_uom.addEventListener('click', (function() {
                    var options = {
                        'uom_list': orderline.product.uom_id
                    };
                    self.gui.show_popup('product_multi_uom_popup',options);
            	}.bind(this)));
            }
    		return el_node
    	},
...
//    Fixed and Percentage Discount Functionality
        ...
        button_click: function(){
            ...
            this.gui.show_popup('DiscountPopupWidget',{
                ...
                'confirm': function(val, type) {
                    ...
                    self.apply_discount(val,type);
                },
            });
        },
        apply_discount: function(pc,type) {
            var order    = this.pos.get_order();
            var lines    = order.get_orderlines();
            var product  = this.pos.db.get_product_by_id(this.pos.config.order_discount_product[0]);
    ...
    screens.define_action_button({
        ...
        'condition': function(){
            return this.pos.config.enable_order_discount && this.pos.config.order_discount_product;
        },
    });
});
