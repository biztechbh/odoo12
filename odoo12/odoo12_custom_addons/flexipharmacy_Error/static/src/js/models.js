odoo.define('flexipharmacy.models', function (require) {
	...
	models.load_fields("res.users", ['image_small','login','company_ids','access_ereceipt','access_quick_cash_payment',
	                                 ...
	                                 'access_pos_graph','access_x_report','access_pos_loyalty','is_delivery_user','access_create_so_order',
	                                 'access_today_sale_report','access_money_in_out','access_gift_card','access_gift_voucher',
	                                 'access_create_po_order','access_default_customer','rfid_no',
	                                 ...
	                                 'access_pos_dashboard','access_product_expiry_report','shop_ids','display_own_sales_order']);
	models.load_fields("res.partner", ['display_name','prefer_ereceipt','remaining_loyalty_points',
	                                   ...
	                                   'remaining_loyalty_amount', 'loyalty_points_earned','birth_date','anniversary_date',
	                                   'total_remaining_points','remaining_wallet_amount','credit_limit','is_doctor',
	                                   'remaining_credit_limit','remaining_debit_amount','debit_limit','property_product_pricelist']);
	models.load_fields("product.product", ['sequence','qty_available','type','is_packaging','product_brand_id','loyalty_point',
	                    'write_date','is_dummy_product','return_valid_days','non_refundable','invoice_policy',
	                    'attribute_value_ids', 'lst_price']);
	models.load_fields("product.category", ['complete_name']);
	models.load_fields('pos.session',['cash_register_id','is_lock_screen','current_cashier_id','locked','locked_by_user_id','opening_balance','shop_id']);
    ...
    models.load_fields("pos.order", ['order_make_picking'], ['picking_id']);
    models.load_fields("pos.config", ['min_percentage_field']);
...
//        ids:    function(self){ return [self.config.stock_location_id[0]]; },
        ...
        loaded: function(self,records){
            self.set({'recurrent_order_list' : records});
        },
    },{
...
//        	return [['from_date','<=',current_date],['to_date','>=',current_date],['active','=',true]];
	...
	function decimalAdjust(value){
	    ...
	    if(_.contains(_.range(0,50), reminder_value)){
	        rounding_value = eval(split_value[0].toString() + '.' + division_value.toString() + '0' )
	    }else if(_.contains(_.range(50,100), reminder_value)){
	...
	models.PosModel = models.PosModel.extend({
	    ...
	    load_server_data: function () {
            ...
            self.product_domain = [["sale_ok","=",true],["available_in_pos","=",true]];
	        return posmodel_super.load_server_data.apply(this, arguments).then(function () {
	        	...
	        	rpc.query(params_ir_config, {async: false})
		    	.then(function(system_parameters){
	    			if(system_parameters){
	    				if(system_parameters && system_parameters.length){
	    					if(system_parameters[0].google_api_key){
	    						self.system_parameters = system_parameters.google_api_key;
    						}
...
//	        	var from_cache = false;
            	if(self.config.multi_store_id && self.config.multi_store_id[0] && self.pos_session.store_id && self.pos_session.store_id[0]){
...
//                $('.modal-footer').css({position: absolute,right: 10px,bottom: 10px});
//                $('.modal-dialog .close').css({position: absolute;right:11px});
...
//            }
            ...
            var currenct_date = utc.toJSON().replace("T", " ").split(".")[0];
            var params = {
                model: 'stock.quant',
                method: 'search_read',
                domain: [['product_id', '=', product.id],
                 ['lot_id.name','=',parsed_code.product_lot_code],
                 ['quantity', '>', 0],
                 ['lot_id.life_date', '>=', currenct_date],
            ...
            return rpc.query(params, {async: false}).then(function(serials){
                if(serials.length > 0){
                    var lot_info = _.filter(serials, function(serial){
                        return parsed_code.product_lot_code === serial.lot_id[1]
                    });
...
//						domain_list.push(['shop_id','=',self.config.multi_shop_id[0]])
						...
						domain_list = [['id','in',server_ids],['shop_id','=',false]]
					}
					var params = {
						model: 'pos.order',
						method: 'ac_pos_search_read',
...
// Order Summary
        ...
        add_product: function(product, options){
        	...
        	if(this.get_missing_mode()){
    		...
    		}else if(options && options.lot_number){
        	    if(this._printed){
                    this.destroy();
                ...
                if(line.has_product_lot){
                    if(options.lot_number){
                        this.direct_lot_selection(options.lot_number)
                    }else{
...
//            		  var change = this.get_total_paid() - this.getNetTotalTaxIncluded() - this.get_order_total_discount();
    	...
    	display_lot_popup: function() {
            ...
            if(order_line && order_line.product.type == "product"){
            	var pack_lot_lines =  order_line.compute_lot_lines();
            	console.log("pack_lot_lines", pack_lot_lines);
            	if(pack_lot_lines.length === 0){
            	    order_line.pack_lot_lines.add(new models.Packlotline({}, {'order_line': order_line}));
                    this.set_quantity_by_lot(pack_lot_lines)
            	}
            	var product_id = order_line.get_product().id;
            	if(this.pos.config.enable_pos_serial){
                    self.open_custom_pack_lot_popup(self, pack_lot_lines, product_id, self.pos.config.stock_location_id[0])
                } else {
        ...
        open_custom_pack_lot_popup: function(order, pack_lot_lines, product_id, location_id){
            var self = this;
            if(product_id){
                var params = {
                    model: 'stock.production.lot',
                    method: 'get_lot_information',
                    args:[[],product_id, location_id]
                }
                rpc.query(params, {async: false}).then(function(serials){
                    if(serials.length > 0){
                        console.log("HERE 1111", pack_lot_lines);
                        self.pos.gui.show_popup('packlotline', {
                            'title': _t('Lot/Serial Number(s) Required'),
                            'pack_lot_lines': pack_lot_lines,
                            'order': order,
                        ...
                        self.pos.get_order().remove_orderline(self.pos.get_order().selected_orderline)
                        self.pos.db.notification('danger',_t('No item found for the current Location'));
                    }
                });
            }
        },
    ...
    models.Orderline = models.Orderline.extend({
        ...
        export_for_printing: function() {
            ...
            var new_attr = {
            	...
            	product_name:this.get_product().display_name +"-"+this.get_product().default_code,
        ...
        generate_wrapped_product_name: function() {
            ...
            var wrapped = [];
            var name = this.get_product().display_name+"-"+this.get_product().default_code;
            var current_line = "";
