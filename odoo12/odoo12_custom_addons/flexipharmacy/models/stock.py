...
class stock_picking(models.Model):
    ...
    def do_detailed_internal_transfer(self, vals):
        ...
        if vals and vals.get('data'):
            ...
            if picking_id:
                for i, move in enumerate(picking_id.move_lines):
                    if move.product_id.tracking == 'lot' or move.product_id.tracking == 'serial':
                        qty_done = 0
                        pack_lots = []
                        # for k, each in enumerate(move_lines):
                        #     print("\n\n????????", i, "--", k, "==", each[2])
                        pack_lot_names = [each.get('lot_name') for each in move_lines[i][2].get('lot_info')]
...
class StockMove(models.Model):
    ...
    cost = fields.Float(string='Rate', digits=(12, 3), related="product_id.standard_price")
    total = fields.Float(string='Amount', digits=(12, 3), readonly=True,
                         compute='_compute_total')

    @api.depends('cost')
    def _compute_total(self):
        for each in self:
            each.total = each.cost * each.product_uom_qty
