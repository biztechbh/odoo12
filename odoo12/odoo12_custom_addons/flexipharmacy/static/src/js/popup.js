odoo.define('flexipharmacy.popup', function (require) {
	...
	var ProductNotePopupWidget = PopupWidget.extend({
	    ...
	    events : _.extend({}, PopupWidget.prototype.events, {
	    	'focus #textarea_note' : 'focus_textarea_note',
    		'focusout #textarea_note' : 'focusout_textarea_note',
	    }),
	    focus_textarea_note: function(event){
	    	$('body').off('keypress', self.pos.chrome.screens.payment.keyboard_handler);
	        $('body').off('keydown',self.pos.chrome.screens.payment.keyboard_keydown_handler);
	    },
	    focusout_textarea_note: function(event){
	    	$('body').off('keypress', self.pos.chrome.screens.payment.keyboard_handler).keypress(self.pos.chrome.screens.payment.keyboard_handler);
	        $('body').off('keydown',self.pos.chrome.screens.payment.keyboard_handler).keydown(self.pos.chrome.screens.payment.keyboard_keydown_handler);
	    },
	    show: function(options){
	        options = options || {};
	        this._super(options);
...
//	    		self.gui.show_screen('products');
	...
	var CreateCardPopupWidget = PopupWidget.extend({
        ...
        show: function(options){
                ...
                source: function(request, response) {
                    ...
                    if(query){
                        search_timeout = setTimeout(function(){
                            var partners_list = [];
                            var clients = self.pos.db.search_gift_card_customer(query);
            ...
            $("#text_amount").keypress(function (evt) {
                var $txtBox = $(this);
...
//            	self.pos.db.notification('danger',_t('Press enter key.'));
    ...
    var RechargeCardPopupWidget = PopupWidget.extend({
        ...
        show: function(options){
            ...
            $("#text_recharge_amount").keypress(function (evt) {
                var $txtBox = $(this);
                var charCode = (evt.which) ? evt.which : evt.keyCode
                if (charCode > 31 && (charCode < 48 || charCode > 57) && charCode != 46)
                    return false;
...
//				self.pos.db.notification('danger',_t("Press enter to get voucher amount"));
	...
	var AddToWalletPopup = PopupWidget.extend({
	    ...
	    click_confirm: function(){
	    	var self = this;
	    	var order = self.pos.get_order();
	    	var cash_register_id = self.pos.pos_session.cash_register_id;
	    	if(!self.pos.config.cash_control){
	    		self.pos.db.notification('danger',_t("Please enable cash control from point of sale settings."));
	    		return;
	    	}
	    	if(!cash_register_id){
                self.pos.db.notification('danger',_t("There is no cash register for this PoS Session."));
	    		return;
	    	}
	    	if(order.get_client()){
	    ...
	    validate: function(){
	    	...
	    	self.pos.chrome.screens.payment.finalize_validation();
	...
	var DeliveryDatePopup = PopupWidget.extend({
	    ...
	    show: function(options){
		    ...
		    if(options){
		        ...
		        this.from_draft_order = options.from_draft_order;
		    }
		    var order = this.pos.get_order();
			self.renderElement();
            ...
            if(!self.from_draft_order){
            	...
            	if(order && self.to_be_update_order.delivery_date != $('#delivery_datepicker').val()){
                	var params = {
                		model: 'pos.order',
                		method: 'update_delivery_date',
                		args: [self.to_be_update_order.id, $('#delivery_datepicker').val(), true]
                	}
                	rpc.query(params, {async: false})
		            .then(function(res){
		                self.pos.db.add_orders(res);
    ...
    var MaxCreditExceedPopupWidget = PopupWidget.extend({
        ...
        click_override_payment: function(){
        	...
        	if(self.options.payment_obj){
        	    if(currentOrder.get_reservation_mode() && !currentOrder.get_paying_due() && !currentOrder.get_cancel_order()){
...
//                }
	...
	var MaxDebitExceedPopupWidget = PopupWidget.extend({
	    template: 'MaxDebitExceedPopupWidget',
	    show: function(options){
	        var self = this;
	        this._super(options);
	    },
	...
	gui.define_popup({name:'max_debit_limit', widget: MaxDebitExceedPopupWidget});
	...
	var CashControlWizardPopup = PopupWidget.extend({
        ...
        renderElement: function() {
            ...
            var table_row = "<tr id='cashcontrol_row'>" +
                            "<td><input type='text'  class='cashcontrol_td coin' id='value' value='"+self.format_currency_no_symbol(0)+"' /></td>" + "<span id='errmsg'/>"+
                            "<td><input type='text' class='cashcontrol_td no_of_coin' id='no_of_values' value='"+self.format_currency_no_symbol(0)+"' /></td>" +
                            "<td><input type='text' class='cashcontrol_td subtotal' id='subtotal' disabled='true' value='"+self.format_currency_no_symbol(0)+"' /></td>" +
    ...
    var PackLotLinePopupWidget = PopupWidget.extend({
                ...
                for(var i=0; i < item.pack_lot_lines.length; i++){
                    ...
                    if(serials.length != 0){
                        for(var j=0 ; j < serials.length ; j++){
                            if(serials[j].name == lot_line.get('lot_name')  && serials[j].product_id[0] == item.product.id){
	    ...
	    click_confirm: function(){
	        ...
	        this.$('.packlot-line-input').each(function(index, el){
	            var cid = $(el).attr('cid'),
	                lot_name = $(el).val();
	            var lot_info = _.filter(self.serials, function(serial){
	                return $(el).val() === serial.name
	            });
...
//	        $("#loading").hide();
	    ...
	    click_confirm: function(){
            ...
            if(!supplier){
	        	self.pos.db.notification('danger',"Please select Supplier!");
	        	return
			}
...
//	Multi-Store Popup
	...
	var AddMoneyToCreditPopup = PopupWidget.extend({
	    ...
	    show: function(options){
            ...
            $("#amount-to-be-added").keypress(function (evt) {
                var $txtBox = $(this);
                var charCode = (evt.which) ? evt.which : evt.keyCode
                if (charCode > 31 && (charCode < 48 || charCode > 57) && charCode != 46)
                    return false;
	    ...
	    click_confirm: function(){
            ...
            rpc.query(params, {async: false}).then(function(vals){
                if(vals){
                	...
                	if (self.pos.config.iface_print_via_proxy) {
                        var receipt = "";
                        receipt = QWeb.render('CustomAddedCreditXmlReceipt', {
                            widget: self,
                            pos: self.pos,
                            order: order,
                            receipt: order.export_for_printing(),
...
//  Credit Management
    var PrintCustomerCreditDetailPopup = PopupWidget.extend({
        ...
        show: function(options){
        	var self = this;
        	self._super(options);
        	$('.start-date-ledger input').focus();
        },
        click_confirm: function(){
            var self = this;
            var order = self.pos.get_order();
            var start_date = $('.start-date-ledger input').val();
            var end_date = $('.end-date-ledger input').val();
            self.pos.credit_from_date = start_date;
            self.pos.credit_end_date = end_date;
            ...
            if(start_date && end_date){
                ...
                .then(function(vals){
                    if(vals){
                        if(partner && vals.length > 0){
                            ...
                            if (self.pos.config.iface_print_via_proxy) {
                                var receipt = "";
                                receipt = QWeb.render('AddedCreditStatementXmlReceipt', {
                                    widget: self,
                                    pos: self.pos,
                                    order: order,
                                    receipt: order.export_for_printing(),
                ...
                $('.start-date-ledger input').css({'border-style': 'solid','border-width': '1px',
                    'border-color': 'rgb(255, 0, 0)'});
                $('.end-date-ledger input').css({'border-color': 'rgb(224,224,224)'});
            }else if(end_date == "" && start_date !== ""){
                $('.end-date-ledger input').css({'border-style': 'solid','border-width': '1px',
                    'border-color': 'rgb(255, 0, 0)'});
                $('.start-date-ledger input').css({'border-color': 'rgb(224,224,224)'});
            }else{
                $('.start-date-ledger input, .end-date-ledger input').css({'border-style':'solid', 'border-width': '1px',
                    'border-color': 'rgb(255, 0, 0)'});
            }
        },
    });
    ...
    var PayDebutPopup = PopupWidget.extend({
	    ...
	    show: function(options){
            ...
            $("#amount-to-be-pay").keypress(function (evt) {
                var $txtBox = $(this);
                var charCode = (evt.which) ? evt.which : evt.keyCode
                if (charCode > 31 && (charCode < 48 || charCode > 57) && charCode != 46)
                    return false;
    ...
    var InternalTransferPopupWidget = PopupWidget.extend({
        ...
        click_confirm: function(){
            ...
            _.each(currentOrderLines,function(item) {
               ...
               data['state'] = 'draft';
               if(item.has_product_lot){
                    item.pack_lot_lines.each(_.bind( function(lot) {
                        return pack_lot_ids.push(lot.export_as_JSON());
                    }, this));
               }
               data['lot_info'] = pack_lot_ids
               moveLines.push(data);
            });
...
//	        $('#orderdate-datepicker').focus();
	        ...
	        if(order.get_edit_quotation()){
	            if(order.get_sale_order_date()){
                    ...
                    $('#orderdate-datepicker').val(date_order.locale('en').format('YYYY-MM-DD HH:mm'));
                }
	            if(order.set_sale_order_requested_date()){
	                var date_order = moment(order.get_sale_order_requested_date(), 'YYYY-MM-DD HH:mm');
                    $('#requesteddate-datepicker').val(date_order.locale('en').format('YYYY-MM-DD HH:mm'));
                }
            ...
            $('textarea.sale_order_note').focus(function() {
                if(self.payment_obj && self.pos.gui.get_current_screen() == 'payment'){
	    ...
	    click_confirm: function(){
            ...
            var sale_order_date = $('#orderdate-datepicker').val();
            var request_order_date = $('#requesteddate-datepicker').val();
            if(sale_order_date > request_order_date){
                alert("Sale order date should not be greater than Commitment date");
                return
            }
	    ...
	    renderElement: function(){
	        var self = this;
	        this._super();
			$('#orderdate-datepicker').datetimepicker({format:'Y-m-d H:i'}).val(new moment ().locale('en').format("YYYY-MM-DD HH:mm"));
			var order_date = $('#orderdate-datepicker').val();
			$('#requesteddate-datepicker').datetimepicker({
				format:'Y-m-d H:i',
				minDate: new Date(order_date),
				minTime: (order_date ? order_date.split(' ')[1] : false),

			}).val(new moment ().locale('en').format("YYYY-MM-DD HH:mm"));
            $(".tabs-menu-sale a").click(function(event) {
                event.preventDefault();
                $(this).parent().addClass("current_tab");
                $(this).parent().siblings().removeClass("current_tab");
                var tab = $(this).attr("href");
                $(".sale-order-tab-content").not(tab).css("display", "none");
                $(tab).fadeIn();
            });
    ...
    var DeliveryDetailPopupWidget = PopupWidget.extend({
        template: 'DeliveryDetailPopupWidget',
         events: _.extend({}, PopupWidget.prototype.events, {
            'click #emptybox_delivery_detail_date': 'emptybox_delivery_detail_date',
		 	'click #emptybox_delivery_detail_time':'emptybox_delivery_detail_time',
         }),
		 emptybox_delivery_detail_date:function(el){
		 	$('#delivery_detail_date').val('')
		 },
		 emptybox_delivery_detail_time:function(el){
		 	$('#delivery_detail_time').val('')
		 },
		show: function(options){
            var self = this;
        	options = options || {};
            this._super(options);
            self.deliver_options = options;
            var order = this.pos.get_order();
            $("#delivery_detail_date").focus(function() {
            	$('body').off('keypress', self.pos.chrome.screens.payment.keyboard_handler);
            	$('body').off('keydown',self.pos.chrome.screens.payment.keyboard_keydown_handler);
            });
            $("#delivery_detail_date").focusout(function() {
            	$('body').off('keypress', self.keyboard_handler).keypress(self.pos.chrome.screens.payment.keyboard_handler);
		        $('body').off('keydown',self.keyboard_keydown_handler).keydown(self.pos.chrome.screens.payment.keyboard_keydown_handler);
            });
            $("#delivery_detail_time").focus(function() {
            	$('body').off('keypress', self.pos.chrome.screens.payment.keyboard_handler);
            	$('body').off('keydown',self.pos.chrome.screens.payment.keyboard_keydown_handler);
            });
            $("#delivery_detail_time").focusout(function() {
            	$('body').off('keypress', self.keyboard_handler).keypress(self.pos.chrome.screens.payment.keyboard_handler);
		        $('body').off('keydown',self.keyboard_keydown_handler).keydown(self.pos.chrome.screens.payment.keyboard_keydown_handler);
            });
            $("#delivery_detail_address").focus(function() {
            	$('body').off('keypress', self.pos.chrome.screens.payment.keyboard_handler);
            	$('body').off('keydown',self.pos.chrome.screens.payment.keyboard_keydown_handler);
            });
            $("#delivery_detail_address").focusout(function() {
            	$('body').keypress(self.pos.chrome.screens.payment.keyboard_handler);
		        $('body').off('keydown',self.keyboard_keydown_handler).keydown(self.pos.chrome.screens.payment.keyboard_keydown_handler);
            });
            if(order.get_is_delivery()){
            	if(order.get_delivery_date()){
            		$('#delivery_detail_date').val(order.get_delivery_date());
            	}
            	if(order.get_delivery_time()){
            		$('#delivery_detail_time').val(order.get_delivery_time());
            	}
            	if(order.get_delivery_address()){
            		$('#delivery_detail_address').val(order.get_delivery_address());
            	} else if(order.get_client()){
            		$('#delivery_detail_address').val(order.get_client().address);
            	}
            	if(order.get_delivery_user_id()){
            		$('.delivery_detail_user').val(order.get_delivery_user_id())
            	}
            	$("#delivery_detail_date").datepicker({
            		minDate: 0,
            		onSelect: function(dateText, inst) {
            			order.set_delivery_date(dateText);
            		},
            	});
    	        $("#delivery_detail_time").timepicker({
    	        	'timeFormat': 'h:i A',
    	        });
    	        $('#delivery_detail_time').on('changeTime', function() {
    	        	order.set_delivery_time($(this).val());
    	        });
            }
        },
        click_confirm: function(){
        	var self = this;
        	var order = self.pos.get_order();
        	var date = $("#delivery_detail_date").val();
        	var time = $("#delivery_detail_time").val();
        	var address = $('#delivery_detail_address').val();
        	var delivery_user_id = $('.delivery_detail_user').val();
            ...
            if(self.deliver_options && self.deliver_options.call_from && self.deliver_options.call_from == 'debit_button'){
            	self.pos.chrome.screens.payment.pos_debit();
            }
            if(self.deliver_options && self.deliver_options.call_from && self.deliver_options.call_from == 'draft_order'){
            	self.pos.chrome.screens.products.create_draft_order();
            }
        	self.gui.close_popup();
        }
    });
    gui.define_popup({name:'delivery_detail_popup', widget: DeliveryDetailPopupWidget});
    ...
    // Create Recurrent Order
    var CreateRecurrentOrderPopupWidget = PopupWidget.extend({
        template: 'CreateRecurrentOrderPopupWidget',
        show: function(options){
            var self = this;
            self.rec_partner_id = '';
            self.rec_prod_id = '';
            this._super(options);
            this.renderElement();
            $('#select_recurrent_customer').focus();
            $('#select_recurrent_customer').autocomplete({
                source: function(request, response) {
                    var query = request.term;
                    var search_timeout = null;
                    if(query){
                ...
                select: function(event, partner) {
                    ...
                    // event.preventDefault();
                    if (partner.item && partner.item.id) {
                        self.rec_partner_id =  partner.item.id;
                        var partner_obj = _.find(self.partners, function(customer) {
                            return customer.id == partner.item.id;
                        });
                        if (partner_obj) {
            ...
            $("#recurrent_no_of_days").keypress(function (e) {
                if (e.which != 8 && e.which != 0 && (e.which < 48 || e.which > 57) && e.which != 46) {
                    return false;
               }
            });
        ...
        click_confirm: function(){
            var self = this;
            var rec_prod_details = [];
            var rec_cust_id = Number(self.rec_partner_id);
            var rec_no_of_days = Number($("#recurrent_no_of_days").val());
            var execution_date = $('#execution_date').val();
            var pos_user = Number($('#select_rec_pos_user').val());
            var rec_is_deliver = $('#recurrent_order_deliver').prop('checked');
            var rec_cust_add = $('#recurrent_delivery_add').val();
            var tableHtml=$('#tbl_recurrent_product_items');
            tableHtml.find('tbody > tr').each(function(){
                var price = $(this).find('td #prod_price_rec').val();
                var qty = $(this).find('td #prod_qty_rec').val();
                var product = $(this).find('td .rec_product_id').attr('id');
                if(product){
                    rec_prod_details.push([0,0,{'product_id':Number(product),
                                        'qty':Number(qty),
                                        'unit_price':Number(price)}])
                }
            });
            if(!rec_cust_id){
                self.pos.db.notification('danger',_t('A Customer Is Required'));
            } else if(!rec_no_of_days){
                self.pos.db.notification('danger',_t('A Number of Days Is Required'));
            } else if(!pos_user){
                self.pos.db.notification('danger',_t('A POS User Is Required'));
            } else if(!execution_date){
                self.pos.db.notification('danger',_t('A Execution Date Is Required'));
            } else{
                var params = {
                    model: "pos.recurrent.order",
                    method: "create",
                    args: [{
                        'partner_id': rec_cust_id,
                        'pos_config_id': self.pos.config.id,
                        'pos_user_id': pos_user,
                        'next_exe_date': execution_date,
                        'active': true,
                        'is_deliver': rec_is_deliver,
                        'deli_address': rec_cust_add,
                        'no_of_day': rec_no_of_days,
                        'product_line': rec_prod_details,
                    }]
                }
                rpc.query(params, {async: false}).then(function(result){
                    if(result){
                        self.gui.close_popup();
                        self.pos.db.notification('success',_t('Automatic refill medicine saved successfully.'));
                        self.pos.chrome.screens.recurrent_order_screen.reloading_recurrent_recs();
                    }
                }).fail(function(){
                    self.pos.db.notification('danger',"Connection lost");
                });
        ...
        renderElement: function() {
            var self = this;
            this._super();
            $('.execution_datetime').datepicker({
            	minDate: 0,
            	dateFormat:'yy/mm/dd',
            });
            $('#select_recurrent_customer').on('change', function() {
                var rec_cust_id = Number(self.rec_partner_id);
                if(rec_cust_id){
                    var partner_data = self.pos.db.get_partner_by_id(rec_cust_id);
                    if(partner_data){
                        var street = partner_data.street || '';
                        var city = partner_data.city || '' ;
                        var state_id = partner_data.state_id[1] || '';
                        var zip = partner_data.zip || '';
                        var country_id = partner_data.country_id[1] || '';
                        $('#recurrent_delivery_add').val(street + ' ' + city + ' ' + state_id + ' ' + zip
                            +' ' + country_id)
                    }
                }
            });
            $("#recurrent_no_of_days").on('change', function() {
                var rec_no_of_days = Number($("#recurrent_no_of_days").val());
                if(rec_no_of_days){
                    var currentDate = new Date();
                    currentDate.setDate(currentDate.getDate()+rec_no_of_days);
                    currentDate = $.datepicker.formatDate('yy/mm/dd', currentDate);
                    $('.execution_datetime').val(currentDate);
                }
            });
            $('#recurrent_order_deliver').click(function(){
                if($(this).prop("checked") == true){
                    $(".del_add").css("display", "contents");
                }
                else if($(this).prop("checked") == false){
                    $(".del_add").css("display", "none");
                }
            });
            var selectedOrder = self.pos.get_order();
            var recurrent_tbl_row = "<tr id='recurrent_tbl_row'>" +
                            "<td><input type='text'  class='recurrent_td rec_product_id' id='prod_id_rec'/></td>" + "<span id='errmsg'/>"+
                            "<td><input type='text' class='recurrent_td rec_prod_qty' id='prod_qty_rec' /></td>" +
                            "<td><input type='text' class='recurrent_td rec_prod_price' id='prod_price_rec' disabled='true'/></td>" +
                            "<td id='rec_delete_row'><span class='fa fa-trash-o'></span></td>" +
                            "</tr>";
            $('#tbl_recurrent_product_items tbody').append(recurrent_tbl_row);
            $('#add_new_recurrent_product').click(function(){
                $('#tbl_recurrent_product_items tbody').append(recurrent_tbl_row);
            });
            $('#tbl_recurrent_product_items tbody').on('click', 'tr#recurrent_tbl_row td#rec_delete_row',function(){
				$(this).parent().remove();
			});
            $('#tbl_recurrent_product_items tbody').on('change focus', 'tr#recurrent_tbl_row td',function(){
                $('.recurrent_td.rec_product_id').autocomplete({
                    source: function(request, response) {
                        var query = request.term;
                        var search_timeout = null;
                        if(query){
                            search_timeout = setTimeout(function(){
                                var product_list = [];
                                var products = self.pos.db.search_product_in_category(0,query);
                    ...
                    select: function(event, product) {
                        ...
                        if (product.item && product.item.id) {
                            self.rec_prod_id = product.item.id;
                            $('#prod_id_rec').attr('id', product.item.id);
                            var product_obj = _.find(self.products, function(prod) {
                                return prod.id == product.item.id;
                            });
                        }
                    },
                    focus: function(event, ui) {
            ...
            $('#tbl_recurrent_product_items tbody').on('change focusout', 'tr#recurrent_tbl_row td',function(){
                var product,product_price;
                if(self.rec_prod_id){
                    var product_data = self.pos.db.get_product_by_id(Number(self.rec_prod_id))
                    if(product_data){
                        $(this).parent().find('td #prod_price_rec').val(product_data.list_price);
                    }
                }
            });
            $("#prod_qty_rec").keypress(function (e) {
                if (e.which != 8 && e.which != 0 && (e.which < 48 || e.which > 57) && e.which != 46) {
                    return false;
                }
            });
    ...
    gui.define_popup({name:'create_recurrent_order_popup', widget: CreateRecurrentOrderPopupWidget});
    ...
    var ProductMultiUomPopupWidget = PopupWidget.extend({
        template: 'ProductMultiUomPopupWidget',
        init: function(parent, args) {
            this._super(parent, args);
            this.options = {};
            this.uom_list = [];
        ...
        show: function(options){
			...
			this._super();
			options = options || {};
			$(".template_list").on("click",function(e){
				$(".template_list").removeClass('selected');
				$(this).addClass('selected');
			});
            var current_uom = self.pos.units_by_id[options.uom_list[0]];
            this.current_uom = options.uom_list[0];
            var uom_list = self.pos.units_by_id;
            var uom_by_category = self.pos.get_order().get_units_by_category(uom_list, current_uom.category_id);
            self.uom_list = uom_by_category;
            self.render_list(self.uom_list);
            $('.product-uom-list tr').eq(1).addClass('selected');
            $(".template_list").on("click",function(e){
				$(".template_list").removeClass('selected');
				$(this).addClass('selected');
			});
			$(".button.apply").on('click',function(){
			    var uom_id = parseInt($('.template_list.selected').attr('id'));
			    var order = self.pos.get_order();
			    order.set_custom_uom_id(uom_id);
			    var res = order.apply_uom();
			    if(res){
			    	self.gui.close_popup();
			    } else{
        ...
        render_list: function(uom_list){
	        var contents = this.$el[0].querySelector('.product_uom_list_contain');
	        contents.innerHTML = "";
	        for(var i=0;i<uom_list.length;i++){
	            var report_list = uom_list[i];
                var reportsline_html = QWeb.render('ProductUomLine',{widget: this, uom_line:uom_list[i]});
	    ...
	    renderElement: function(){
	    	var self = this;
	    	self._super();
	    	var discount_id = parseInt($('.template_list.selected').attr('id'));
	    },
    });
    gui.define_popup({name:'product_multi_uom_popup', widget: ProductMultiUomPopupWidget});

//    var posmodel_super = models.PosModel.prototype;
//    var NumberPopupWidget = _.filter(posmodel_super.gui.popup_classes, function(popup){
//        return popup.name = 'number';
//    });
//    console.log("\n\n\n>>>>>>>>>NumberPopupWidget.widget",NumberPopupWidget.widget)
////    NumberPopupWidget.widget.extend({
////        click_confirm: function(){
////            alert()
////            this.gui.close_popup();
////            if( this.options.confirm ){
////                var discount_val = self.$('input[name=discount_type]:checked').val()
////                this.options.confirm.call(this,this.inputbuffer, discount_val);
////            }
////        },
////    });
    var DiscountPopupWidget = PopupWidget.extend({
        ...
        click_confirm: function(){
            this.gui.close_popup();
            if( this.options.confirm ){
                var discount_type = this.$('input[name=discount_type]:checked').val()
                this.options.confirm.call(this,this.inputbuffer,discount_type);
            }
        },
    });
    gui.define_popup({name:'DiscountPopupWidget', widget: DiscountPopupWidget});
});
